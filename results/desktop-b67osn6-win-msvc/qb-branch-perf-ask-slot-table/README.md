...
against `qb/` itself at `1ec7e794` (0 dirty). The branch is measured at its head only here — the
intermediate `8f28e7bf` was A/B'd on WSL2, where the profile lives.

| directory | qb at | what |
|---|---|---|
| `grid-a61bded8/`, `grid-a61bded8-pass2/` | `develop` `1ec7e794` + `8f28e7bf` (the slot table: `[core:16][generation:26][slot:22]`, FIFO free list, no hashing) + **`a61bded8`** (take binds the slot in the same call, `finish()` runs once, the owner is built before the send) — **the candidate**, measured first and third | **4 cells**, qb only, all verified: bank-transaction × {1c-spin, 1c-park, 2c-spin, 2c-park}. |
| `grid-1ec7e794/` | `develop` `1ec7e794` — **the control**, measured second | same 4 cells, same session. |
...
| config | `1ec7e794` | **`a61bded8`** | pass 2 | Δ | min: ctl / cand / pass 2 |
|---|---|---|---|---|---|
...
supports, recorded and not profiled on this host: the cost `a61bded8` removes on g++ is the
...
| `dev/bench` cell (median of 3 medians) | ctl `1ec7e794` → `a61bded8` |
|---|---|
