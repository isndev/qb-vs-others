...
measured against the `develop` it forks from (`f8eba11d`, the 3.2.0 candidate grid's build) on
...
against the working tree, `build/ab180-ctl` against `D:\repo\qb-ctl-f8eba11d`, a clean clone at
`f8eba11d`, 0 dirty; shipped 3.1.0 is `build/shipped-win`), candidate / control / shipped /
candidate in ONE quiet session on 2026-09-07 with the WSL2 side idle (its own session had ended
10:11 UTC):

| directory | qb at | what |
|---|---|---|
| `grid-0f7994e6/`, `grid-0f7994e6-pass2/` | **the branch head `0f7994e6`** — measured first and fourth | **12 cells** each, qb only, all verified: three shapes × {1c-spin, 1c-park, 2c-spin, 2c-park}. 10:17:17–10:18:35 UTC. |
| `grid-f8eba11d/` | `develop` `f8eba11d` — the control, measured second | same 12 cells, same session. |
...
| `census/` | `0f7994e6` vs `f8eba11d`, **10 interleaved launches** each, 3 reps + 1 warmup, on the four counting cells (the grid's pass 2 was elevated there), the two 2c-spin cells and the 1c ping-pong anchor | 10:22:31–10:23:15 UTC. |
...
for BOTH builds** — 1c-spin `f8eba11d` 8.6 … 12.6 (9.0) against `0f7994e6` 8.8 … 12.7 (9.5), 1c-park
