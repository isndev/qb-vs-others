...
against the working tree of `/mnt/d/repo/qb-dev/qb`, `develop` (`00b60afe`, every axis through N
...
`develop` `00b60afe`, measures fib at **43 s** per repetition on Windows (below), which is not a
...
the window and its writer thread shares the two pinned CPUs. `5665b6f8` demotes every one of
...
|---|---|---|
| `develop` `00b60afe` (axis I dense tables, unreleased) | **43.2 s** (Windows/MSVC; Linux not waited for) | the dense router's `key_table` grew by +1 slot per `registerEvent`, rehashing the whole table — five default events per actor × 57 312 actors, O(n²); latent because nothing had ever created more than a few hundred actors per core |
| `5665b6f8` | 178 ms → 28 ms (Windows) | growth by capacity (43 s → 178 ms); then `VirtualCore::addActor`/`removeActor`'s per-actor `LOG_INFO` (114 624 lines inside the window) demoted to `LOG_VERB` |
| `d44e7b44` | **8.49 ms** | `ActorMap` was a hash map keyed by an id that IS a per-core slot, the kill queue a hash set deduplicating what an idempotent `kill()` deduplicates itself, and subscribe paid a `dynamic_cast` per registration (5 per actor) |
| `001be013` | **7.08 ms** | `active_coroutines_` was a `make_shared` in every constructor and a release in every destructor, for actors that never spawn — ~30 % of an actor's lifetime on the profile |
