...
(`f8eba11d`, the candidate grid's build) on the three shapes that sit on that cost —
...
tree; the control `~/qvo/ctl-f8eba11d`, a clean LF clone at `f8eba11d` (0 dirty); shipped is
...
|---|---|---|
| `grid-guard-only/`, `grid-guard-only-pass2/` | `f8eba11d` + the tick-phase guard ALONE (the `LoopEvent` and its snapshot behind `if (!_callback_list.empty())`) — measured first and fourth | **12 cells** each, qb only, all verified: three shapes × {1c-spin, 1c-park, 2c-spin, 2c-park}. Session 1, 09:32:14–09:40:20 UTC. |
| `grid-f8eba11d/` | `develop` `f8eba11d` — the control of session 1, measured second | same 12 cells, same session. |
| `grid-shipped-3.1.0/` | v3.1.0, measured third | same 12 cells, same session. |
| `census-guard-only/` | guard-only vs `f8eba11d`, **10 interleaved launches** each, 3 reps + 1 warmup, on the four 2c cells + the 1c ping-pong anchor | the instrument for the cross-core cells (§9.11); 09:41:26–09:42:24 UTC. |
| `census-variants/` | `f8eba11d` / guard-only / five idle-pass variants, **7 interleaved launches** each — the third of three variant censuses, the one whose JSON survived (the first two were overwritten in `/tmp` and are transcribed below) | what settled the fix; 09:50:22–09:52:02 UTC. |
| `grid-0f7994e6/`, `grid-0f7994e6-pass2/` | **the branch head `0f7994e6`**: the guard + the idle clock read on every idle pass in every latency mode — measured first and fourth | 12 cells each. Session 2, 09:55:27–10:03:31 UTC. |
| `grid-f8eba11d-session2/`, `grid-shipped-3.1.0-session2/` | the control and shipped 3.1.0 of session 2, measured second and third | 12 cells each, same session. |
...
| `census-final/` | `0f7994e6` vs `f8eba11d`, **15 interleaved launches** each, on the three 2c-spin cells, counting 2c-park and the 1c anchor | 10:07:36–10:08:24 UTC. |
...
`perf record -e cpu-clock -F 25000` (no PMU under WSL2) on `f8eba11d`'s ping-pong at one core,
...
`443c5976` ("`time()` samples the clock on demand") had moved the per-pass read from the top of
...
| cell | shipped 3.1.0 | `f8eba11d` | guard only p1 / p2 | Δ vs `f8eba11d` |
|---|---:|---:|---:|---:|
...
(median 252.6) against `f8eba11d` **198.2 … 212.9** (202.2); thread-ring 2c-spin **127.9 …
...
Every variant is `f8eba11d` + the guard + one insertion at the end of an idle spin-mode pass
...
| cell | `f8eba11d` | guard only | + `spin_loop_pause()` |
|---|---:|---:|---:|
...
| cell | `f8eba11d` | guard only | + `pause` | + `lfence` | + tight `has_data()` poll ×256 | + poll with `pause` |
|---|---:|---:|---:|---:|---:|---:|
...
| cell | `f8eba11d` | guard only | + `lfence` | **+ `mono_now()` on the idle pass** | + `lfence` + `pause` | + `lfence` + `rdtsc` |
|---|---:|---:|---:|---:|---:|---:|
...
| cell | shipped 3.1.0 | `f8eba11d` | **`0f7994e6`** p1 / p2 | Δ vs `f8eba11d` | Δ vs shipped |
|---|---:|---:|---:|---:|---:|
