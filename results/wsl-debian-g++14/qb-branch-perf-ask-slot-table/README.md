...
control is `~/qvo/ctl-ba9b1a81`, a clean LF clone at `ba9b1a81` (0 dirty). The branch is two
commits, and each was measured in its own session on 2026-09-07:

| directory | qb at | what |
|---|---|---|
| `grid-dfa303ec/`, `grid-dfa303ec-pass2/` | `develop` `ba9b1a81` + **`dfa303ec`** (the slot table: `[core:16][generation:26][slot:22]`, FIFO free list, no hashing) — the first candidate, measured first and third | **4 cells**, qb only, all verified: bank-transaction × {1c-spin, 1c-park, 2c-spin, 2c-park}. Session A, 06:01:55–06:03:35 UTC. |
| `grid-ba9b1a81/` | `develop` `ba9b1a81` — **the control** of session A, measured second | same 4 cells, same session. |
| `grid-82ea03a8/`, `grid-82ea03a8-pass2/` | `dfa303ec` + **`82ea03a8`** (take binds the slot in the same call, `finish()` runs once, the owner is built before the send) — the branch head, measured first and third | same 4 cells. Session B, 06:18:10–06:19:50 UTC. |
| `grid-ba9b1a81-session-b/` | `ba9b1a81` again — the control of session B, measured second | same 4 cells, same session. |
| `bench-dfa303ec/`, `bench-82ea03a8/` | the two `dev/bench` ask binaries, candidate and control alternated three times (`cand-N-*` / `ctl-N-*`), 5 repetitions each, aggregates only | run right after each session's grids; medians of the three medians below. |
...
| config | session A: `ba9b1a81` | **`dfa303ec`** | pass 2 | Δ | session B: `ba9b1a81` | **`82ea03a8`** | pass 2 | Δ |
|---|---|---|---|---|---|---|---|---|
...
The slot table alone (`dfa303ec`) is inside the spread on one core and −3 % on two; the head of
...
| `dev/bench` cell (median of 3 medians) | session A ctl → `dfa303ec` | session B ctl → `82ea03a8` |
|---|---|---|
...
| symbol | `ba9b1a81` | `dfa303ec` | `82ea03a8` |
|---|---|---|---|
...
`dfa303ec` removed the hashing — and the run moved 1–4 %, less than the 4.4 points the registry
...
on `next != busy`. `82ea03a8` makes it three — `ask_take(owner, slot)` pops and binds in one
