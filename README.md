...
2. **qb's parked mode is bad, and CAF's is not.** 10.71 µs against 511 ns. That is qb losing by
   ...
   thing this repository has produced so far.
3. **CAF barely moves.** 508–511 ns across all four configurations. Its work-stealing pool is
   remarkably insensitive to how it is tuned — which is a real engineering property that a
   ranking column does not show.
